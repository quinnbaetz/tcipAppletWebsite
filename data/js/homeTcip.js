...
$(document).ready(function(){
	...
	$('#navHeader').click(function() {
		 var elem =  $('#navDiv');
		 if (elem.is(":hidden")) {
			$('#navArrow').attr('src', 'data/images/downArrow.png');
			if(iframeShim){
			...
			$('#navArrow').attr('src', 'data/images/rightArrow.png');
			elem.slideUp();
			if(iframeShim){
	...
	$('#expander').click(function() {
		 var elem =  $('#projBar');
		 if (elem.is(":hidden")) {
			$('#expander img').attr('src', 'data/images/collapse.png');
			$('#expander').animate({'margin-left': '194px'});
			elem.css('display', 'block');	
			elem.animate({'width' : '206px'});
		 }else{
			$('#expander img').attr('src', 'data/images/expand.png');
	...
	function focusSideBar(target){
		for(var noti in roots){
			if($(roots[noti]+"Link")[0] !== target){
				...
				$(roots[noti]+"Arrow").attr('src', 'data/images/blueRightArrow.png');
			}else{
				$(roots[noti]+"Arrow").attr('src', 'data/images/blueDownArrow.png');
	...
	for(var i in roots){
		$(roots[i]+"Link").click(function(evt){
			if($(evt.currentTarget).next().css('display') == "none"){
				...
				$(evt.currentTarget).find('img').attr('src', 'data/images/blueRightArrow.png');
			}
		});
	}
